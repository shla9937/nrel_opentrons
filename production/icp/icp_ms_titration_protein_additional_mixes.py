...
metadata = {
    'protocolName': 'ICP-MS - 6 x 12 point 1:1 protein dilution, 96 well plate, additional metal mixes',
    ...
    Stock metals should be at 25µM in proper pH buffer (500µL).
    Protein should be at 200µM in proper pH buffer (1000µL).
    Mixes at 25µM (200µL).
...
def add_buff(protocol):
    ...
    for col in range(0,3): # Test mix control
        p300m.transfer(rxn_vol*(4/5), buff, rxn_plate.rows()[7][col].bottom(10), new_tip='never')
    for col in range(3,6): # Test mix with protein
        p300m.transfer((rxn_vol*(4/5))-(rxn_vol/40), buff, rxn_plate.rows()[7][col].bottom(10), new_tip='never')
    for col in range(6,9): # 3 new mixes control
        p300m.transfer(rxn_vol*(4/5), buff, rxn_plate.rows()[7][col].bottom(10), new_tip='never')
    for col in range(9,12): # 3 new mixes with protein
...
def titrate_protein(protocol):
    ...
    p20.transfer(rxn_vol/40, protein, rxn_plate.rows()[7][3:6], new_tip='always', mix_after=(3,20))
    p20.transfer(rxn_vol/40, protein, rxn_plate.rows()[7][9:12], new_tip='always', mix_after=(3,20))

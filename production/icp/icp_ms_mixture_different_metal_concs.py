...
metadata = {
    'protocolName': 'ICP-MS mixture, can control metal concentrations',
...
def setup(protocol):
    # equiptment
    global p300m, tips300, tips300_1, desalt_plate, res1, rxn_plate, icp_plate, proteins, res2, metal_plate
    ...
    metal_plate = protocol.load_labware('nest_96_wellplate_2ml_deep', 6)
    res2 = protocol.load_labware('nest_1_reservoir_195ml', 8)

    global buff, acid, metal_mix, rxn_vol
    buff = res1.wells()[0]
    acid = res2.wells()[0]
    metal_mix = metal_plate.wells()[0]

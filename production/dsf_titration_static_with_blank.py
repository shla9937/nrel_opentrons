...
metadata = {
    'protocolName': 'DSF metal titration - static, with blank',
...
def titrate(protocol):
    ...
    for metal in range(0, metals):
        p300m.aspirate(220, sypro2)            
        row = metal % 8 
        col = (metal // 8) + (len_titration - 1)
        p300m.dispense(220, metal_plate.rows()[row][col])
    p300m.drop_tip()
...
def add_protein(protocol): # add 10µL of protein
    ...
    for sample in range(0, samples):
        pickup_tips(1, p300m, protocol)
        for metal in range(0, 4):
            p300m.aspirate(60, samples_loc[sample])            
            row = metal % 8 
            col = (metal // 8) + (sample * 6)
            p300m.dispense(60, pcr1.rows()[row][col])
        for metal in range(4, 8):
            p300m.aspirate(60, samples_loc[sample])            
            row = metal % 8 
            col = (metal // 8) + (sample * 6)
            p300m.dispense(50, pcr1.rows()[row][col])
        p300m.drop_tip()

    # pick up 4 tips (eventually change to number of metals) and distrribute
    for sample in range(0, samples):
        pickup_tips(4, p300m, protocol) # change to "metals" later, accounting for > 8
        col = sample * 6
        p300m.distribute(10, pcr1.rows()[3][col],
                    pcr1.rows()[3][col+1:col+len_titration],
                    disposal_volume=0, new_tip='never')
        p300m.distribute(10, pcr1.rows()[7][col],
                    pcr1.rows()[7][col+1:col+len_titration-1],
                    disposal_volume=0, new_tip='never')
        p300m.drop_tip()
...
def add_titration(protocol): # add 10µL of titration
    ...
    # add extra 10ul to blanks
    pickup_tips(4, p20m, protocol) # need to adjust to metals later
    col = len_titration -1
    row = 7

...
metadata = {
    'protocolName': 'DSF screen - 12 point 1:3 dilution, 8 metals',
    'author': 'Shawn Laursen',
    'description': '''
    12 well dilution = 11 well dilution series (1mM -> 1µM) + 0 point
    Use 10µM stock protein -> 5µM final. (10µl x 96) + extra 
    Use 200mM Metal -> 1mM highest, then 4x dilution x 10 (1024 x total) to >1µM.
...
def add_sypro(protocol):
    ...
    p300m.aspirate(180, sypro) 
    for row in range(0, 8):        
        p300m.dispense(20, pcr.rows()[row][0])
    p300m.drop_tip()
...
def titrate(protocol):
    ...
    pickup_tips(8, p300m, protocol)
    p300m.transfer(30,pcr.rows()[0][0:10],pcr.rows()[0][1:11],
                mix_after=(5, 20),new_tip='never')
    p300m.aspirate(30, pcr.rows()[0][10])
    p300m.drop_tip()
